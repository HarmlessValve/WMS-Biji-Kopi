...
{
    /// <summary>
    /// Controller untuk semua operasi Pengiriman Kopi.
    ...
    public class PengirimanController
    {
        ...
        public List<DropdownItem> GetDestinations()
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT destination_id, destination_name FROM vw_active_destinations", conn))
                    using (var reader = cmd.ExecuteReader())
                        while (reader.Read())
                            result.Add(new DropdownItem { Id = reader.GetInt32(0), Name = reader.GetString(1) });
                }
            }
            catch (Exception ex) { Console.WriteLine("GetDestinations error: " + ex.Message); }
            return result;
        }
        ...
        public List<LaporanPengirimanItem> GetDataPengiriman()
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    using (var cmd = new NpgsqlCommand("SELECT tanggal, destinasi, jenis_kopi, jumlah, petugas FROM vw_outgoing_transactions ORDER BY tanggal DESC", conn))
                    using (var reader = cmd.ExecuteReader())
                        while (reader.Read())
            ...
            catch (Exception ex) { Console.WriteLine("GetDataPengiriman error: " + ex.Message); }
            return result;
        }
        ...
        /// <summary>
        /// Memanggil fn_get_or_create_product dan sp_add_outgoing_transaction.
        /// </summary>
        public bool SimpanPengiriman(int destinationId, int coffeeId, int categoryId, int originId, int roastLevelId, int quantity, int petugasId)
        {
            try
            {
                using (var conn = DatabaseHelper.GetConnection())
            ...
            {
                Console.WriteLine("SimpanPengiriman error: " + ex.Message);
                return false;
            }
        }
    }

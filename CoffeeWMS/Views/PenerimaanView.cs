...
namespace CoffeeWMS.Views
{
    public class PenerimaanView : UserControl
    {
        ...
        private readonly PenerimaanController _controller = new PenerimaanController();
        ...
        private void LoadInitialData()
        {
            try
            {
                // Load Supplier (statis)
                BindDropdown(cmbSupplier, _controller.GetSuppliers());

                // Load Jenis Kopi (cascading level 1)
                LoadJenisKopi();
            }
            catch (Exception ex)
            {
        ...
        private void BtnSimpan_Click(object? sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Data Supplier, Jenis, Kategori Kopi, atau Origin belum dipilih!", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (sukses)
            {
                MessageBox.Show("Data penerimaan kopi berhasil disimpan!", "Sukses", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtJumlah.Clear();
                RefreshGrid();
            }
            else
        ...
        {
            var data = _controller.GetDataPenerimaan();
            dgvPenerimaan.DataSource = data.Count > 0 ? (object)data : null;
        }
